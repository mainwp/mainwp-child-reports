...
class MainWP_WP_Stream_Feeds {
	...
	public static function load() {
		...
		if ( ! self::$is_network_feed ) {
			if ( ! isset( MainWP_WP_Stream_Settings::$options['general_private_feeds'] ) || ! MainWP_WP_Stream_Settings::$options['general_private_feeds'] ) {
				return;
			}
		}
		...
		add_action( 'wp_ajax_mainwp_wp_stream_feed_key_generate', array( __CLASS__, 'generate_user_feed_key' ) );
	...
	public static function generate_user_feed_key() {
		check_ajax_referer( 'mainwp_wp_stream_generate_key', 'nonce' );

		$user_id = mainwp_wp_stream_filter_input( INPUT_POST, 'user', FILTER_SANITIZE_NUMBER_INT );
	...
	public static function save_user_feed_key( $user ) {
		...
		$verify_nonce = isset( $_GET['mainwp_wp_stream_nonce'] ) && wp_verify_nonce( $_GET['mainwp_wp_stream_nonce'], 'mainwp_wp_stream_generate_key' );
	...
	public static function user_feed_key( $user ) {
		if ( ! is_network_admin() && ! array_intersect( $user->roles, MainWP_WP_Stream_Settings::$options['general_role_access'] ) ) {
			return;
		}
		...
		$nonce = wp_create_nonce( 'mainwp_wp_stream_generate_key' );
		?>
		<table class="form-table">
			<tr>
				<th><label for="<?php echo esc_attr( self::USER_FEED_KEY ) ?>"><?php esc_html_e( 'MainWP Child Reports Feeds Key', 'mainwp-child-reports' ) ?></label></th>
				<td>
					<p class="wp-stream-feeds-key">
						<?php wp_nonce_field( 'mainwp_wp_stream_generate_key', 'mainwp_wp_stream_generate_key_nonce' ) ?>
						...
						<small><a href="<?php echo esc_url( add_query_arg( array( self::GENERATE_KEY_QUERY_VAR => true, 'mainwp_wp_stream_nonce' => $nonce ) ) ) ?>" id="<?php echo esc_attr( self::USER_FEED_KEY ) ?>_generate"><?php esc_html_e( 'Generate new key', 'mainwp-child-reports' ) ?></a></small>
						<span class="spinner" style="display: none;"></span>
					</p>
					<p class="description"><?php esc_html_e( 'This is your private key used for accessing feeds of MainWP Child Reports securely. You can change your key at any time by generating a new one using the link above.', 'mainwp-child-reports' ) ?></p>
					<p class="wp-stream-feeds-links">
						<a href="<?php echo esc_url( add_query_arg( array( 'type' => 'xml' ), $link )  ) ?>" class="rss-feed" target="_blank"><?php echo esc_html_e( 'RSS Feed', 'mainwp-child-reports' ) ?></a>
						|
						<a href="<?php echo esc_url( add_query_arg( array( 'type' => 'json' ), $link ) ) ?>" class="json-feed" target="_blank"><?php echo esc_html_e( 'JSON Feed', 'mainwp-child-reports' ) ?></a>
					</p>
				</td>
			</tr>
		</table>
	...
	/**
	 * Output for MainWP Child Reports as a feed.
	 *
	 * @return xml
	 */
	public static function feed_template() {
		$die_title   = esc_html__( 'Access Denied', 'mainwp-child-reports' );
		$die_message = '<h1>' . $die_title .'</h1><p>' . esc_html__( 'You don\'t have permission to view this feed, please contact your site Administrator.', 'mainwp-child-reports' ) . '</p>';
		...
		if ( ! is_super_admin( $user[0]->ID ) ) {
			...
			if ( ! $roles || ! array_intersect( $roles, MainWP_WP_Stream_Settings::$options['general_role_access'] ) ) {
				wp_die( $die_message, $die_title );
			}
		}
		...
		$args = array(
			'blog_id'          => $blog_id,
			'records_per_page' => mainwp_wp_stream_filter_input( INPUT_GET, 'records_per_page', FILTER_SANITIZE_NUMBER_INT, array( 'options' => array( 'default' => get_option( 'posts_per_rss' ) ) ) ),
			'search'           => mainwp_wp_stream_filter_input( INPUT_GET, 'search' ),
			'object_id'        => mainwp_wp_stream_filter_input( INPUT_GET, 'object_id', FILTER_SANITIZE_NUMBER_INT ),
			'ip'               => mainwp_wp_stream_filter_input( INPUT_GET, 'ip', FILTER_VALIDATE_IP ),
			'author'           => mainwp_wp_stream_filter_input( INPUT_GET, 'author', FILTER_SANITIZE_NUMBER_INT ),
			'author_role'      => mainwp_wp_stream_filter_input( INPUT_GET, 'author_role' ),
			'date'             => mainwp_wp_stream_filter_input( INPUT_GET, 'date' ),
			'date_from'        => mainwp_wp_stream_filter_input( INPUT_GET, 'date_from' ),
			'date_to'          => mainwp_wp_stream_filter_input( INPUT_GET, 'date_to' ),
			'record__in'       => mainwp_wp_stream_filter_input( INPUT_GET, 'record__in', FILTER_SANITIZE_NUMBER_INT ),
			'record_parent'    => mainwp_wp_stream_filter_input( INPUT_GET, 'record_parent', FILTER_SANITIZE_NUMBER_INT ),
			'order'            => mainwp_wp_stream_filter_input( INPUT_GET, 'order', FILTER_DEFAULT, array( 'options' => array( 'default' => 'desc' ) ) ),
			'orderby'          => mainwp_wp_stream_filter_input( INPUT_GET, 'orderby', FILTER_DEFAULT, array( 'options' => array( 'default' => 'ID' ) ) ),
			'fields'           => mainwp_wp_stream_filter_input( INPUT_GET, 'fields', FILTER_DEFAULT, array( 'options' => array( 'default' => 'with-meta' ) ) ),
		);
		...
		$records_admin_url = add_query_arg(
			...
			admin_url( MainWP_WP_Stream_Admin::ADMIN_PARENT_PAGE )
		);

		if ( 'json' === mainwp_wp_stream_filter_input( INPUT_GET, self::FEED_TYPE_QUERY_VAR ) ) {
			...
			>
				<channel>
					<title><?php bloginfo_rss( 'name' ) ?> - <?php esc_html_e( 'MainWP Child Reports Feed', 'mainwp-child-reports' ) ?></title>
