...
class MainWP_WP_Stream_Install {
	 ...
	 *
	 * @access public
	 * @return bool|object|MainWP_WP_Stream_Install
	 */
	...
	function __construct() {
		...
		self::$stream_url = self_admin_url( MainWP_WP_Stream_Admin::ADMIN_PARENT_PAGE . '&page=' . MainWP_WP_Stream_Admin::SETTINGS_PAGE_SLUG );
...
//		}
	...
	public static function prompt_update() {
		...
		<div class="error">
			<form method="post" action="<?php echo esc_url( remove_query_arg( 'mainwp_wp_stream_update' ) ) ?>">
				...
				<p><strong><?php esc_html_e( 'Stream Database Update Required', 'mainwp-child-reports' ) ?></strong></p>
				<p><?php esc_html_e( 'Stream has updated! Before we send you on your way, we need to update your database to the newest version.', 'mainwp-child-reports' ) ?></p>
	 ...
	 * use the mainwp_wp_stream_update_{version_number} version number must be a string of characters that represent the version with no periods
	 *
	 * @filter mainwp_wp_stream_db_update_versions
	 *
	 * @access private
	 * @return array
	 */
...
//		require_once MAINWP_WP_STREAM_INC_DIR . 'db-updates.php';
//
//		$versions = self::db_update_versions();
//
...
//			}
//
//			$function = 'mainwp_wp_stream_update_' . ( 'user' === $update_args['type'] ? '' : $update_args['type'] . '_' ) . str_ireplace( '.', '', $version );
//
