...
class MainWP_WP_Stream_Connector_Settings extends MainWP_WP_Stream_Connector {
	...
	public static function get_context_labels() {
		$context_labels = array(
			...
			'mainwp_wp_stream'         => __( 'MainWP Child Reports', 'mainwp-child-reports' ),
		...
		if ( is_network_admin() ) {
			$context_labels = array_merge(
				$context_labels,
				array(
					'mainwp_wp_stream_network'  => __( 'MainWP Child Reports Network', 'mainwp-child-reports' ),
					'mainwp_wp_stream_defaults' => __( 'MainWP Child Reports Defaults', 'mainwp-child-reports' ),
				)
			);
		}
	...
	public static function get_field_label( $field_key ) {
		$labels = array(
			...
			'user_count'                    => __( 'User Count', 'mainwp-child-reports' ),
			// Other
			'mainwp_wp_stream_db'                  => __( 'MainWP Child Reports Database Version', 'mainwp-child-reports' ),
		);
	...
	public static function get_serialized_field_label( $option_name, $field_key ) {
		 ...
		 */
		$labels = apply_filters( 'mainwp_wp_stream_serialized_labels', $labels );
	...
	public static function action_links( $links, $record ) {
		...
		$rules = array(
			'stream' => array(
				'menu_slug'    => 'mainwp_wp_stream',
				'submenu_slug' => MainWP_WP_Stream_Admin::SETTINGS_PAGE_SLUG,
				'url'          => function( $rule, $record ) {
					$option_key = mainwp_wp_stream_get_meta( $record->ID, 'option_key', true );
					$url_tab    = null;

					if ( '' !== $option_key ) {
						foreach ( MainWP_WP_Stream_Settings::get_fields() as $tab_name => $tab_properties ) {
				...
				'applicable'   => function( $submenu, $record ) {
					return $record->context === 'mainwp_wp_stream';
				}
			),
			...
			if ( ! empty( $applicable_rules ) ) {
				...
				if ( ! empty( $found_submenus ) ) {
					...
					if ( current_user_can( $capability ) ) {
						$url        = apply_filters( 'mainwp_wp_stream_action_link_url', $url, $record );
						$text       = sprintf( __( 'Edit %s Settings', 'mainwp-child-reports' ), $context_labels[ $record->context ] );
						$field_name = mainwp_wp_stream_get_meta( $record->ID, 'option_key', true );

						if ( '' === $field_name ) {
							$field_name = mainwp_wp_stream_get_meta( $record->ID, 'option', true );
						}
			...
			(function ($) {
				$(function () {
					...
					if (location.hash.substr(1, hashPrefix.length) === hashPrefix) {
						...
						// try to find mainwp_wp_stream field
						if ( $field.length === 0 ) {
							fieldNames = [
								"mainwp_wp_stream_" + hashFieldName,
								"mainwp_wp_stream[" + hashFieldName + "]"
							];
