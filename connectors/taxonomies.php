...
class MainWP_WP_Stream_Connector_Taxonomies extends MainWP_WP_Stream_Connector {
	...
	public static function get_label() {
		return __( 'Taxonomies', 'mainwp-child-reports' );
	}
	...
	public static function action_links( $links, $record ) {
		if ( $record->object_id && 'deleted' !== $record->action && ( $term = get_term_by( 'term_taxonomy_id', $record->object_id, $record->context ) ) ) {
			if ( ! is_wp_error( $term ) ) {
				...
				$links[ sprintf( _x( 'Edit %s', 'Term singular name', 'mainwp-child-reports' ), $tax_label ) ] = get_edit_term_link( $term->term_id, $term->taxonomy );
				$links[ __( 'View', 'mainwp-child-reports' ) ] = get_term_link( $term->term_id, $term->taxonomy );
			}
		}
	...
	public static function _registered_taxonomy( $taxonomy, $object_type, $args ) {
		...
		MainWP_WP_Stream_Connectors::$term_labels['stream_context'][ $taxonomy ] = $label;
	}
